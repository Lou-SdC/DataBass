...
def predict_pyin(lodaed_df_file):
    ...
    with ProcessPoolExecutor(max_workers=mw or 1) as executor:
        with alive_bar(len(rows), title='Predicting with pyin') as bar:
            for result in executor.map(_predict_line_pyin, rows):
                results.append(result)
                bar()

    prediction_dir = os.path.join(working_dir, 'data', 'baseline')
    os.makedirs(prediction_dir, exist_ok=True)
    prediction_file = os.path.join(prediction_dir, 'pyin_notes.csv')
    pd.DataFrame(results).to_csv(prediction_file, index=False)
    return prediction_file

def _predict_line_pyin(row):
    ats = row.get('audio_time_serie', None)
    if ats is None or (isinstance(ats, float) and np.isnan(ats)) or ats == '':
        y = np.array([], dtype=np.float32)
    elif isinstance(ats, (list, np.ndarray)):
    ...
    f0, voiced_flag, _ = librosa.pyin(y, fmin=librosa.note_to_hz('E1'), fmax=librosa.note_to_hz('C5'), sr=sr)
